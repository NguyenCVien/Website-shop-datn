...
public interface OrderDetailDAO extends JpaRepository<OrderDetail, Long> {
	...
	@Query("SELECT * FROM Orders AS OD INNER JOIN Accounts AS Acc ON OD.Username = Acc.Username inner join OrderDetails as odt on OD.OrderId = odt.OrderId WHERE status =?1")
