...
public interface OrderDetailDAO extends JpaRepository<OrderDetail, Long> {
	...
	@Query(value = "SELECT Orders.OrderId, Orders.Username, Products.Name, OrderDetails.Quantity, OrderDetails.orderdetailId, OrderDetails.Price, OrderDetails.Status FROM Orders INNER JOIN OrderDetails ON Orders.OrderId = OrderDetails.OrderId INNER JOIN Products on OrderDetails.ProductId =  Products.ProductId INNER JOIN Accounts on Orders.Username =  Accounts.Username where OrderDetails.Status like N'Đang vận chuyển' and Accounts.Username like ?1", nativeQuery = true)
	List<OrderDetail> findByStatus(String status);
