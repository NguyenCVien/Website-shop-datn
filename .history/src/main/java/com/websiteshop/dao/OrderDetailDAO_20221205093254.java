...
public interface OrderDetailDAO extends JpaRepository<OrderDetail, Long> {
	...
	@Query(value = "SELECT * FROM Order AS OD INNER JOIN Account AS Acc ON OD.Username = Acc.Username inner join OrderDetail as odt on OD.OrderId = odt.OrderId WHERE status =?1", nativeQuery = true)
