...
public class authorityController {
    ...
    public ModelAndView edit(ModelMap model, @PathVariable("id") Integer id) {
        ...
        if (opt.isPresent()) {
            ...
            return new ModelAndView("/authority/addOrEdit", model);
        }
    ...
    public ModelAndView saveOrUpdate(ModelMap model,
        ...
        if (result.hasErrors()) {
            ...
            return new ModelAndView("admin/authority/addOrEdit");
        }
        Authority au = new Authority();
        BeanUtils.copyProperties(dto, au);

        authorityService.save(au);
        model.addAttribute("message", "Tài khoản đã cấp quyền");
        return new ModelAndView("forward:/authority", model);
    }
